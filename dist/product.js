...
document.getElementById('projectFile').onchange=async e=>{
  ...
  try{
    ...
    restoreDesignSides(p.version===2?p.sides:null,sideImages,p.activeSide||0);linkedSideSettings=p.version===2&&!!p.linkSides;$('linkDesignSettings').checked=linkedSideSettings;
    projectStateToUI();updateDevicePresetTip(AppState.preset);if(img)scheduleRebuild(true);else if(templateActive())renderTemplateBlank();updateStats();saveSettings();clean();toast('Project opened','success');
