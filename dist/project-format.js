...
globalThis.ProjectFormat={
  decode(text){
    ...
    if(p.bottomBrand!==undefined){const b=p.bottomBrand;if(!b||typeof b.enabled!=='boolean'||!['underside','inside'].includes(b.surface))throw Error('Invalid bottom branding.');for(const [k,lo,hi]of [['width',4,60],['height',4,36],['centerX',-32,32],['centerZ',-20,20]])if(typeof b[k]!=='number'||!Number.isFinite(b[k])||b[k]<lo||b[k]>hi)throw Error('Invalid bottom branding size or position.');const depthMm=b.depthMm??Math.min(3,Math.max(.1,p.settings.depthMm||.6)),relief=b.relief??(p.settings.relief||'raised');if(typeof depthMm!=='number'||!Number.isFinite(depthMm)||depthMm<.1||depthMm>3||!['raised','carved'].includes(relief))throw Error('Invalid bottom finish.');p.bottomBrand={depthMm,relief,enabled:b.enabled,surface:b.surface,width:b.width,height:b.height,centerX:b.centerX,centerZ:b.centerZ};}
    return p;
  }
};
