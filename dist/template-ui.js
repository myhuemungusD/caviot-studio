...
function syncTemplateUI(){
  ...
  $('templatePlacement').hidden=!active;$('templateToolbar').hidden=!active;$('templateDescription').textContent=AppState.templateId==='etsyfolger-v1'?'Original cavity, openings and dimensions preserved.':'Dimensions generated from the selected elliptical preset.';
...
function ensurePreviewWorker(){
  ...
  templatePreviewWorker.onmessage=({data})=>{templatePreviewRunning=false;if(data.id===templateRevision&&templateActive()){if(data.error){templatePreviewValid=false;templateStatus(data.error,true)}else{templatePreviewValid=true;replaceTemplateMesh(data.positions,data.indices,data.amplitude,data.walls);AppState.lastValidation={triCount:data.indices.length/3,boundary:0,nonManifold:0,zeroArea:0};const extra=(data.info.sharp?' Clean contours enabled.':'')+(data.info.clipped?' Parts near rims or openings are protected.':'');templateStatus(data.info.affected?`${AppState.relief==='carved'?'Deboss':'Emboss'} follows the sleeve normals.${extra}`:'No relief is on the sleeve. Move the design or adjust background removal.',!data.info.affected);updateStats()}}dispatchPreview()};
